Below is a revised version of the tutorial with the critical step—updating **app/index.js**—moved into an early, dedicated section so it isn’t overlooked. This revision makes it clear from the start that you must replace the default contents of **app/index.js** with our custom HomeScreen code.

---

# Complete Functional Tutorial for ScriptHammer

This guide builds your mobile app framework using Expo Router, Supabase, NativeWind, Storybook, and Jest testing. The Supabase backend is set up first, and then you initialize your Expo project using a modern folder structure (the **app/** directory replaces the old **App.js**). Every file is created with complete, functional code blocks. The tutorial also includes a terminal script to automate file creation.

> **Important:**  
> – Run the Supabase SQL commands in your Supabase dashboard before beginning the Expo project.  
> – **Critical Step:** You must replace the default contents of **app/index.js** with the provided code—this is explained in detail in Section 2.1.
...
2. [Initial Expo Setup & Critical HomeScreen Update](#2-initial-expo-setup--critical-homescreen-update)  
   2.1. [Updating app/index.js](#21-updating-appindexjs)  
...
Run these SQL commands in your Supabase dashboard to create a table for user roles and a trigger that assigns roles (the first user becomes “admin”, subsequent users are assigned “user”):
...
Initialize your Expo project with Expo Router by running these commands:
...
### 2.2 **Critical Step: Update app/index.js**

The default **app/index.js** file generated by Expo Router must be replaced with our custom HomeScreen.  
**Do the following:**

1. **Open `app/index.js`** in your code editor.
2. **Delete all existing content** (from line 1 to the end).
...
This step is **critical** to ensure your home route displays our custom components instead of the default content.
...
Below is the shell script that creates the complete project structure and files automatically. (Run the Supabase SQL commands manually in your Supabase dashboard before running this script.) An `exit 0` is added at the end so the script terminates automatically.
...
After following this tutorial, your project should have a structure similar to this:
...
## 9. IMPORTANT: Updating Default Contents of app/index.js

This is the critical step that must not be overlooked. When your Expo Router project is created, a default **app/index.js** file is generated. **You must delete all of its default content and replace it with the following code**:
...
Make sure to perform this change starting at line 1, so that your HomeScreen displays our custom components instead of any default content.

---

By following this revised tutorial—with the critical step to update **app/index.js** moved into an early, dedicated section—you will have built a robust mobile app framework that:

- **Integrates Supabase** for backend authentication, role management, and data storage.
- **Uses Expo Router** with a modern **app/** directory (with your HomeScreen in **app/index.js** and Admin screen in **app/admin/index.js**).
...
This version explicitly instructs you to update **app/index.js** immediately after the project is set up, ensuring that the default content is replaced with our intended HomeScreen code. Happy coding!
