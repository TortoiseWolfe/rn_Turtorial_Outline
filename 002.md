1. **Using a cross‑platform storage adapter for Supabase:**  
   On web, we now use `window.localStorage` instead of AsyncStorage (which is native‑only).

2. **Adjusting file uploads for web:**  
   When uploading images (both in posts and profile updates), we convert the file URI to a Blob on web so that Supabase Storage receives proper file data.

Please copy each file exactly as shown and follow the folder structure. Also, ensure your environment variables are set as indicated.
...
```bash
npm install zustand nativewind @supabase/supabase-js @react-native-async-storage/async-storage expo-image-picker react-native-url-polyfill
```
...
*(Updated for web: converts image URI to Blob for uploads)*

```tsx
import { useState, useEffect } from 'react';
import { Platform, View, Text, TextInput, Image, FlatList, Pressable, Alert } from 'react-native';
  ...
  const submitPost = async () => {
    ...
    try {
      if (image) {
        ...
        let fileData: any;
        if (Platform.OS === 'web') {
          // Convert the image URI to a Blob on web
          const response = await fetch(image.uri);
          fileData = await response.blob();
        } else {
          fileData = { uri: image.uri, name: filePath, type: `image/${image.ext}` };
        }
        const { error: uploadError } = await supabase
          .storage
          .from('posts')
          .upload(filePath, fileData, { contentType: `image/${image.ext}` });
...
*(Updated for web: converts avatar image URI to Blob for upload)*

```tsx
import { useState, useEffect } from 'react';
import { Platform, View, Text, TextInput, Image, Pressable, Alert } from 'react-native';
import * as ImagePicker from 'expo-image-picker';
import { supabase } from '../../supabase';
import { useAuthStore } from '../../store';
    ...
    if (!result.canceled && result.assets.length > 0) {
      ...
      try {
        ...
        if (Platform.OS === 'web') {
          ...
          fileData = { uri: file.uri, name: filePath, type: file.type || 'image/jpeg' };
        }
        const { error: uploadError } = await supabase
          .storage
          .from('avatars')
          .upload(filePath, fileData, { contentType: file.type || 'image/jpeg' }, { upsert: true });
        if (uploadError) throw uploadError;
...
- **Environment Variables:**  
  Ensure that all environment variables are named exactly as:  
...
- **Push Notifications:**  
  Configure Expo Notifications and Supabase Edge Functions separately.

- **Integration & Maintainability:**  
  Always maintain the integrity of your existing system by following your coding standards. Use platform checks (via `Platform.OS`) to isolate web‑specific code, and test thoroughly on both mobile (Expo Go) and web (using `expo start --web`).

This complete code base now covers all core functionality for your MVP Social Media App and should work on both mobile and web without crashing. Happy coding!
