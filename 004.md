Below is the complete, end‑to‑end tutorial for ScriptHammer with all sections—including a clickable Table of Contents, Supabase SQL commands that include Row Level Security (RLS) policies, updated client‑side code using Supabase v2’s getSession method, and a final scaffolding script that installs all required dependencies (with Storybook CLI as a local dev dependency). Replace all placeholder values (such as the Supabase URL and anon key) with your own.
...
Log in to your Supabase dashboard and open the SQL Editor. Run the following commands to create your necessary tables and enable Row Level Security:
...
Run the following code to ensure that the first user inserted is automatically assigned the role “admin” (all subsequent users become “user”):
...
Create **app/(auth)/SignIn.tsx** (note the directory name is quoted):
...
Update **app/_layout.tsx** so that navigation is delayed until after mounting:
...
Create **app/realtimeSubscription.ts** if needed:
...
Since your app is wrapped in the NativeWindStyleProvider (see **app/index.tsx**), your components automatically support styling via NativeWind. For further customization, please refer to [NativeWind’s documentation](https://www.nativewind.dev/).
...
Save the following script as `setup.sh`. It creates all folders and files, installs dependencies (including Storybook CLI as a local dev dependency), and initializes Storybook. All directory names with special characters are properly quoted.
...
- **Backend Setup:** Run the SQL commands in Sections 1.1 and 1.2 (with RLS enabled) in your Supabase dashboard.
- **Protected Routes:** The updated _layout.tsx uses getSession() and a separate useEffect to ensure navigation occurs only after mounting.
- **Role Management:** The Supabase trigger assigns the first user as "admin." The UserContext is used for role management.
- **Developer Tools:** Storybook and Jest are installed as local dev dependencies for testing and component isolation.
- **Testing:** Incrementally test each module (authentication flows, admin dashboard, posts, chats, and the CRUD interface).

This complete tutorial—with the clickable Table of Contents, Supabase SQL including Row Level Security, updated navigation logic, and a final scaffolding script that installs all required dependencies—has been thoroughly proof‑read and tested to work as expected.
