Below is a complete, end‑to‑end tutorial written in Markdown. In this version, explicit HTML anchor tags are added to each section header so that the Table of Contents links are clickable on GitHub and similar Markdown viewers. This tutorial covers:

- Setting up your Supabase backend (with SQL commands for tables, RLS policies, and a trigger to auto‑assign the first user as admin)
- Creating an Expo SDK‑52 project that uses Expo Router with protected groups
- Using the out‑of‑the‑box app.json to supply your Supabase URL and anon key
- Writing a working Supabase client, authentication context, all required pages—including a landing page (app/index.tsx) that redirects based on the authentication state
- A complete shell script (at the end) that bootstraps your project by deleting default files, installing dependencies, and writing all required files

> **Important:**  
> - Replace the placeholder Supabase URL and anon key with your own values (in app.json and in the shell script).  
> - If you encounter issues with folder names containing parentheses (e.g. `(auth)`), you can rename them (e.g. to `auth`) and update the import paths accordingly.  
...
1. [Supabase Backend Setup](#supabase-backend-setup)  
   1. [SQL Commands to Create Tables and RLS Policies](#sql-commands-to-create-tables-and-rls-policies)  
   2. [User Auto‑Creation Trigger with Role Assignment](#user-auto-creation-trigger-with-role-assignment)
...
5. [Routing with Expo Router (Protected & Admin Groups)](#routing-with-expo-router-protected--admin-groups)  
   1. [Protected Routes Layout](#protected-routes-layout)  
   2. [Admin Routes Layout](#admin-routes-layout)
...
7. [Social Network Features](#social-network-features)  
   1. [Home Feed & Creating Posts](#home-feed--creating-posts)  
...
## <a id="supabase-backend-setup"></a>1. Supabase Backend Setup

### <a id="sql-commands-to-create-tables-and-rls-policies"></a>1.1 SQL Commands to Create Tables and RLS Policies
...
### <a id="user-auto-creation-trigger-with-role-assignment"></a>1.2 User Auto‑Creation Trigger with Role Assignment

Run the following SQL to create a trigger that automatically creates a user record when a new user signs up. The first user is assigned the role `"admin"` and subsequent users default to `"user"` (unless specified in user metadata):
...
## <a id="expo-project-setup--environment-variables"></a>2. Expo Project Setup & Environment Variables
...
Then, edit your **app.json** so that its `extra` field looks exactly like this (using double quotes):
...
## <a id="supabase-client-configuration"></a>3. Supabase Client Configuration
...
// Read the environment variables from app.json extra field
...
## <a id="authentication-context--global-layout"></a>4. Authentication Context & Global Layout

### Auth Provider

Place the authentication context in **app/context/AuthProvider.tsx** (note: we’re using a folder without parentheses so that it isn’t treated as a route):
...
Create **app/_layout.tsx**. This file wraps all routes with the AuthProvider:
...
## <a id="routing-with-expo-router-protected--admin-groups"></a>5. Routing with Expo Router (Protected & Admin Groups)
...
## <a id="authentication-screens-login--signup-with-visual-feedback"></a>6. Authentication Screens (Login & Signup) with Visual Feedback
...
## <a id="social-network-features"></a>7. Social Network Features

### Home Feed & Creating Posts
...
## <a id="admin-dashboard"></a>8. Admin Dashboard
...
## <a id="root-route--appindextsx"></a>9. Root Route – app/index.tsx

This file is the landing page. It immediately redirects to the login screen if no user is logged in, or to the home screen if a user is logged in.
...
## <a id="automated-setup-bash-script"></a>10. Automated Setup Bash Script

Below is a complete shell script that bootstraps your project. It deletes default files, installs dependencies, creates the folder structure, and writes all required files. (File paths with parentheses are wrapped in quotes to avoid shell syntax errors.)

Save this as `setup.sh` in your repository’s root, make it executable, and run it:
...
## <a id="final-notes--next-steps"></a>11. Final Notes & Next Steps

You now have a complete, fully functional framework that includes:

- **Supabase SQL Setup:**  
  - Tables for users (with role), posts, comments, chat messages, and notifications with proper RLS policies.  
...
- **Expo Setup:**  
  ...
  - A working `app/index.tsx` that redirects based on the user's authentication state.

- **Social Features:**  
  - A home feed with realtime updates.
  - A Create Post screen (with image upload support via Expo ImagePicker and Supabase Storage).
...
- **Visual Feedback:**  
  - Login and Signup screens display error messages and a loading spinner for immediate user feedback.

- **Automation:**  
  - A complete shell script (`setup.sh`) that bootstraps your project by creating the project, removing default files, installing dependencies, and writing all required files.
...
4. Once the script completes, change directory into **ScriptHammer** and start your project:
   ...
   ```
5. Verify that:
   - If no user is logged in, the landing page (app/index.tsx) redirects to `/ (auth)/login`.
   - If a user is logged in, they are redirected to `/ (protected)/home`.

Happy coding—and enjoy building your full‑featured social network and admin dashboard with Expo SDK‑52 and Supabase!

---

*Tip: If the Table of Contents links do not appear clickable in your Markdown viewer, try viewing the file on GitHub or another viewer that supports explicit HTML anchor tags.*
