Below is a complete, end‑to‑end tutorial in Markdown. When saved as (for example) `TUTORIAL.md` and viewed in a Markdown viewer that supports GitHub-flavored Markdown (such as GitHub itself), the Table of Contents links will be clickable. (Make sure you use a viewer that supports anchor links.) This tutorial walks you through:
...
- Writing a working Supabase client, authentication context, all required pages, and a landing page (`app/index.tsx`) that redirects based on authentication state  
- An automated shell script (at the end) that creates the project, removes default files, installs dependencies, and writes all file contents

> **Important:**  
> - Replace the Supabase URL and anon key with your own values (in `app.json` and the shell script).  
> - If you experience issues with folder names containing parentheses (e.g. `(auth)`), you can rename them (e.g. to `auth`) and update import paths accordingly.  
> - After changes, restart your Expo/Metro bundler.
...
Then, edit your **app.json** so that its `extra` field (using double quotes) looks like this:
...
Place the authentication context in **app/context/AuthProvider.tsx** (using a folder name without parentheses so it is not treated as a route):
...
Create **app/(protected)/_layout.tsx**. (If you experience issues with parentheses in folder names, rename the folder and update the paths accordingly.)
...
- **Expo Setup:**  
  ...
  - A working `app/index.tsx` that immediately redirects based on the user's authentication state.
  - The authentication context is stored in `app/context` (without parentheses) to ensure module resolution works correctly.
...
- **Visual Feedback:**  
  - The Login and Signup screens display error messages and a loading spinner so that users receive immediate visual feedback.
...
4. Once the script completes, change directory into **ScriptHammer** and start your project:
   ...
   ```
5. Verify that:
   - When no user is logged in, the landing page (app/index.tsx) automatically redirects to `/ (auth)/login`.
...
*If your Markdown viewer does not render clickable links in the Table of Contents, try viewing the file on GitHub or another Markdown viewer that supports anchor links.*
