Below is a complete, end‑to‑end tutorial written in Markdown with a clickable Table of Contents. You can copy–paste this text into a Markdown file (for example, `TUTORIAL.md`). When viewed in a Markdown viewer (or on GitHub), the Table of Contents links will be clickable.

> **Important:**  
> – Replace the Supabase URL and anon key with your own values.  
> – If you encounter issues with folder names containing parentheses (e.g. `(auth)`), you can rename them (for example, to `auth`) and update the import paths accordingly.  
> – After making changes, be sure to restart your Expo/Metro bundler.  
...
Create **app/(protected)/_layout.tsx** (if your folder name contains parentheses, ensure your shell or file system supports it):
...
This file serves as the landing page and immediately redirects based on the authentication state.
...
Below is a complete shell script that bootstraps your project. (Note: File paths with parentheses are wrapped in quotes to avoid shell syntax errors.) Save this as `setup.sh` in your repository’s root, make it executable, and run it.
...
- **Expo Setup:**  
  ...
  - A working **app/index.tsx** file that immediately redirects based on the user's authentication state.  
...
5. Verify that:
   - When no user is logged in, **app/index.tsx** automatically redirects to `/ (auth)/login`.
...
*If the clickable Table of Contents does not work in your editor, try viewing the file on GitHub or another Markdown viewer that supports automatic anchor links.*
