Below is an updated, complete, end‑to‑end tutorial. In this version, the authentication screens provide visual feedback by displaying error messages and a loading indicator so that users can see if something goes wrong (for example, if credentials are invalid or the backend isn’t reached).

> **Important:**  
> – Replace the Supabase URL and anon key with your own values.  
> – Read through the tutorial and adjust any paths if needed.  
> – This tutorial uses the new Supabase v2 client methods (for example, `signInWithPassword` instead of the deprecated `signIn`).  
> – After bootstrapping the project, you’ll have clear on‑screen feedback when a login or signup error occurs.
...
9. [Automated Setup Bash Script](#automated-setup-bash-script)

10. [Final Notes & Next Steps](#final-notes--next-steps)
...
> **Note:** The default Expo project may include files such as `app/index.tsx` and `app/_layout.tsx`. Delete these so your custom files are used:
>
> ```bash
> rm -f app/index.tsx
...
### 4.1 Auth Provider (placed in a non‑routing group)

To prevent Expo Router from treating our context file as a route, place it in a folder named `(context)`. Create **app/(context)/AuthProvider.tsx**:
...
// Export default to satisfy Expo Router's requirement
export default AuthProvider;
```

### 4.2 Global Layout

Create **app/_layout.tsx** (the underscore file is used as the global layout):
...
Below, the login and signup screens now include state for an error message and a loading state. Error messages are displayed as text on the screen (in addition to showing an alert) so that users get immediate visual feedback.
...
Below is a complete shell script that bootstraps your project by creating the project (named **ScriptHammer**), deleting default files, installing dependencies, creating the required folder structure, and writing all required files. (Replace the Supabase URL and anon key with your own if needed.)

Save this as `setup.sh` in your repository’s root:
...
- **Expo Setup:**  
  ...
  - Non‑route files (like the AuthProvider) are stored in a `(context)` folder.
...
- **Visual Feedback:**  
  - The Login and Signup screens now display error messages and a loading spinner so that users have clear on‑screen feedback if something goes wrong.
