Below are three complete files that together set up your backend and front‑end environment:

1. **supabase-setup.sql** – Creates the complete ScriptHammer schema (tables, triggers, and RLS policies).  
2. **supabase-dummy-data.sql** – Inserts dummy data (dummy non‑admin user, posts, chats, etc.) after the admin has been set up.  
3. **008.sh** – A Bash script that sets up your Expo project (with the updated Sign‑In screen that shows inline error messages).

Make sure to run the schema script first so that your admin user is created by your auth flow, then later run the dummy data script (using a separate command) once the admin is in place.

---

## 1. supabase-setup.sql  
*(Run this first in your Supabase SQL editor to create the schema.)*
...
-- ===============================================================
-- End of ScriptHammer Supabase Schema Setup
```

---

## 2. supabase-dummy-data.sql  
*(Run this separately after the admin is set up so as not to conflict with the first admin user.)*

> **Note:**  
> This script inserts a dummy non‑admin user (with a fixed UUID distinct from the admin) along with dummy posts, a chat, chat membership, and a message. Adjust the UUID values if needed.

```sql
-- ===============================================================
-- ScriptHammer Dummy Data Insertion
-- This script inserts dummy data (users, posts, chats, etc.) for testing.
-- Make sure to run this AFTER your admin and initial user setup.
-- ===============================================================

-- Insert a dummy non-admin user (dummy user)
-- Ensure that the auth.users table already contains this user before running this script.
INSERT INTO public.profiles (id, username, email, role, created_at)
VALUES ('00000000-0000-0000-0000-000000000002', 'dummyuser', 'dummy2@example.com', 'user', now())
ON CONFLICT (id) DO NOTHING;

-- Insert a dummy post associated with the dummy user.
INSERT INTO public.posts (user_id, content, created_at, updated_at)
VALUES ('00000000-0000-0000-0000-000000000002', 'This is a dummy post from the dummy user.', now(), now());

-- Insert a dummy chat (direct chat)
INSERT INTO public.chats (is_group, created_at)
VALUES (false, now());

-- Assuming this is the second chat created (chat id = 2),
-- insert a dummy chat membership to link the dummy user to the dummy chat.
INSERT INTO public.chat_members (chat_id, user_id, is_admin, joined_at)
VALUES (2, '00000000-0000-0000-0000-000000000002', false, now());

-- Insert a dummy message into the dummy chat from the dummy user.
INSERT INTO public.messages (chat_id, sender_id, content, created_at)
VALUES (2, '00000000-0000-0000-0000-000000000002', 'Hello, this is a dummy message in the chat!', now());
```

---

## 3. 008.sh  
*(This Bash script automates the Expo project setup, including environment variables, dependency installation, and file scaffolding with updated authentication error handling.)*
...
# Sign In screen with inline error messages for why login fails
...
export default function SignInScreen() {
  ...
  // Local state for inputs and error message
  ...
  const handleSignIn = async () => {
    ...
    if (error) {
      // Set inline error message if login fails
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: '#1e90ff', padding: 12, borderRadius: 4, alignItems: 'center' },
...
const styles = StyleSheet.create({
  ...
  linkText: { color: '#1e90ff', textAlign: 'center' },
});
EOF
...
1. **Run the Schema Script:**  
   – Copy and execute **supabase-setup.sql** in your Supabase SQL editor or preferred Postgres client.  
   – This creates your tables, triggers, and RLS policies.  
   – (The admin user is automatically created via your auth flow when the first user signs up.)

2. **Add Dummy Data:**  
   – Once your admin and initial users are set up, run **supabase-dummy-data.sql** in your Supabase SQL editor.  
   – This script inserts a dummy non‑admin user (with a fixed UUID distinct from the admin), a dummy post, a dummy chat, chat membership, and a dummy message.

3. **Set Up the Expo App:**  
   – Ensure your environment variables are set (e.g., `EXPO_PUBLIC_SUPABASE_URL` and `EXPO_PUBLIC_SUPABASE_ANON_KEY`).  
   – Run **008.sh** from your terminal to create and configure the Expo project.  
   – This script installs dependencies, sets up your file structure (including the Sign‑In screen that shows inline error messages), and finally launches the Expo development server with the Metro cache cleared.

By separating the schema and dummy data into two SQL scripts, you avoid any conflicts with the initial admin user and maintain a clear workflow. Happy coding!
