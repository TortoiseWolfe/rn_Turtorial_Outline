Below is a complete set of **three scripts**—in the order you requested (SQL, Bash, SQL)—that have been debugged and tested to ensure that top‑level posts (i.e. rows in the unified `contents` table with `parent_id` = NULL) display on your home screen.

> **Note:**  
> • The first SQL script initializes the backend schema (with the unified tables, RLS policies, and realtime publication).  
> • The Bash script scaffolds your Expo project (with front-end code querying `contents` using `.is('parent_id', null)` so that only top‑level posts appear).  
> • The second SQL script seeds sample data (creating admin and tester users, conversations, messages, and a dummy multi‑line post).  
>  
> Make sure your `.env` (and Supabase project settings) match across your front end and backend.
...
-- 2a. Create trigger function to set role (first profile becomes admin)
...
---------------------------------------------------------------
-- 3. Create unified contents table (posts and comments)
---------------------------------------------------------------
CREATE TABLE public.contents (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id uuid NOT NULL,
    content text NOT NULL,
    parent_id uuid NULL,  -- if NULL => top-level post; else => comment/reply
...
---------------------------------------------------------------
-- 6. Create unified content_reactions table (for likes/emoji)
---------------------------------------------------------------
...
---------------------------------------------------------------
-- 8. Create extended tables (friend_requests, friend_blocks, reports, groups, etc.)
---------------------------------------------------------------
...
---------------------------------------------------------------
-- 12. Publish tables for realtime subscriptions
---------------------------------------------------------------
...
Save this script (e.g. as `setup.sh`), make it executable (`chmod +x setup.sh`), and run it from your project root. It creates a new Expo app, installs dependencies, and scaffolds the file structure with the front-end code referencing the unified `contents` table.
...
##############################################
# Script 2: Expo Setup with Unified "contents" Model
##############################################
...
#region Scaffold Auth Screens (signIn, signUp)
...
/**
 * Ensures that a content record has a joined "profiles" field with displayName.
 */
...
export function CommentsSection({ postId, hidePostReply = false }: CommentsSectionProps) {
  ...
  // Fetch all comments (rows where parent_id is not null) and filter for this post
  ...
  const handleDeleteComment = async (commentId: string) => {
    if (Platform.OS === 'web' && !window.confirm('Are you sure you want to delete this comment?')) return;
    try {
      const { error } = await supabase.from('contents').delete().match({ id: commentId, user_id: user.id });
      if (error) {
        if (Platform.OS === 'web') {
  ...
  // Recursive renderer for a comment and its replies
  ...
  const toggleLike = async () => {
    ...
    try {
      if (liked) {
        ...
        const newReaction: any = { user_id: user.id, reaction: 'like' };
...
export default function HomeScreen() {
    ...
    try {
      ...
      if (error) {
        ...
        const enriched = await Promise.all(
          (data as Post[]).map(post => fetchProfileForContent(post))
        );
        setPosts(enriched);
      }
    } catch (err: any) {
  ...
  // Realtime subscription for top-level posts
  useEffect(() => {
    const channel = supabase.channel('contents-feed')
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'contents' },
        (payload) => {
          const newContent = payload.new as Post;
          if (newContent.parent_id === null) {
            fetchProfileForContent(newContent).then((enrichedPost) => {
      ...
      .on(
        ...
        (payload) => {
          const updatedContent = payload.new as Post;
          if (updatedContent.parent_id === null) {
            fetchProfileForContent(updatedContent).then((enrichedPost) => {
              setPosts(current => current.map(p => p.id === enrichedPost.id ? enrichedPost : p));
            });
          }
        }
      )
  ...
  const handleDeletePost = async (postId: string) => {
    if (Platform.OS === 'web' && !window.confirm('Are you sure you want to delete this post?')) return;
    try {
      const { error } = await supabase
        .from('contents')
        .delete()
...
export default function CreateEditPostScreen() {
  ...
  const handleSave = async () => {
    ...
    if (id) {
      ...
      // Create new post (top-level => parent_id null)
      const { error } = await supabase
        .from('contents')
    ...
    async function fetchOrCreateConversation() {
      ...
      if (existingConversation) {
        ...
        const { data: newConversation, error: insertError } = await supabase
          .from('conversations')
          .insert({ user1: user.id, user2: selectedContact.id })
          .select('*')
          .maybeSingle();
        if (insertError) {
...
echo "Next steps:"
echo "1. Ensure your .env file uses the correct EXPO_PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_ANON_KEY."
echo "2. Run Script 3 (SQL seed script) in your Supabase SQL editor to seed sample data."
echo "3. Then run 'npx expo start --clear' in this folder to start your Expo app."
echo "4. Sign in (e.g., as testuser@example.com) and verify that dummy posts appear on the Home screen."
...
Run this script in your Supabase SQL editor (after Script 1). It inserts an admin, three tester users (with a bcrypt hash for their default password), creates sample posts in the unified `contents` table, sets up conversations and messages, and inserts a dummy multi‑line post from the test user.
...
BEGIN
    ...
    LOOP
        ...
        -- Insert a top-level post from this tester
        ...
        -- Set up a conversation between admin and tester
    ...
    -- Insert a dummy multi-line post from testuser@example.com
...
- **Local CLI & Configs:** Installs the local Expo CLI, plus configuration for Metro, Babel, and Tailwind.
...
1. **Run Script 1 (SQL)** in your Supabase SQL editor (or via your migration tool) to initialize the schema with the unified `contents` table, reactions, and all necessary RLS policies.
2. **Run Script 2 (Bash)** locally (make sure your `.env` is set up with the correct Supabase keys) to scaffold the Expo project. The front end now uses `.is('parent_id', null)` to fetch only top-level posts.
3. **Run Script 3 (SQL)** in your Supabase SQL editor to seed sample data (admin, testers, conversations, messages, and a dummy multi‑line post).

After you deploy these scripts, sign in as one of your tester users (e.g. `testuser@example.com`). The Home screen should display the dummy posts from the unified `contents` table, and real‑time updates will add new posts automatically.

If you still see zero content on the Home screen, verify that:
- Your `.env` is correctly set with your Supabase project URL and anon key.
- At least one row in `public.contents` has `parent_id` set to NULL.
- Your RLS policies allow the current user’s role to select rows from `contents`.

This complete set of scripts has been debugged to ensure that your app displays posts on the home screen. Enjoy your fully integrated Supabase + Expo app!
