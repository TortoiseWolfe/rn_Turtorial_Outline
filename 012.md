Below are the three complete and updated scripts. In this version, replies are inserted into the unified **contents** table with their **parent_id** set to the parent post’s ID, and the Home screen forces a refresh of its nested comments (via a refresh counter passed into the CommentsSection) so that replies appear nested under the parent post.
...
> **Note:** In this version, the LikeButton component uses a transparent background so the emoji appears without an outline. Also, the Home screen has been updated to include a refresh counter for nested comments. When you tap **Reply** on a post, an input appears and when you submit, the reply is inserted into the **contents** table with its **parent_id** set to the parent post’s id—allowing it to appear nested under the parent.
...
# Note: This component now queries the unified "contents" table for replies by checking parent_id.
...
# Note: Added a commentsRefresh counter to force the CommentsSection to re-fetch its data when a reply is submitted.
...
export default function HomeScreen() {
  ...
  // Counter to trigger CommentsSection refresh
  ...
  // When replying, insert a reply into "contents" with its parent_id set to the post's id.
  const handleSubmitPostReply = async () => {
    ...
    try {
      const payload = {
        ...
        parent_id: activePostReplyId, // Reply's parent_id is the post's id being replied to
      };
      const { error } = await supabase.from('contents').insert([payload]).single();
      if (error) {
        ...
        // Trigger CommentsSection refresh by incrementing counter
  ...
  const renderPost = ({ item }: { item: Post }) => {
    ...
    return (
      <View className="p-4 border-b border-gray-300">
        ...
        <CommentsSection postId={item.id} hidePostReply={true} refresh={commentsRefresh} />
        {postErrors[item.id] && (
...
After running these scripts, sign in as one of your testers (for example, `testuser@example.com`) and verify that the Home screen displays the dummy top‑level posts (rows in “contents” where `parent_id` is null). When you tap the **Reply** button on a post, an input will appear; when you submit your reply, it is inserted with its **parent_id** set to the parent post’s id and then the nested comments refresh so the reply appears as a comment under that post.
