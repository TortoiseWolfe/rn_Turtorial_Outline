Below is an updated version of the three complete scripts. In this version, we've moved the reply functionality entirely into the CommentsSection component so that when you reply to a post, the reply is inserted with its parent_id set and then displayed nested underneath its parent. (The Home screen now only shows top‑level posts and renders CommentsSection for each post.) Also, the LikeButton now uses a transparent background so that only the thumbs‑up emoji and its count appear (and the text color will change with light/dark theme).
...
> **Note:** In this version, the reply functionality has been moved entirely into the CommentsSection component so that replies (i.e. comments with a non-null parent_id) are fetched and displayed nested under their parent post. The LikeButton now has a transparent background so that only the thumbs‑up emoji and its count appear, with its text color adapting to light/dark mode. Finally, this script ends by launching Expo with:
>
> ```bash
...
# --- Region: Scaffold Supabase Helpers ---
...
# This component now fetches comments (i.e. replies) for a given post by checking for rows with parent_id equal to the post's id.
...
export function CommentsSection({ postId, hidePostReply = false, refresh = 0 }: CommentsSectionProps) {
  ...
  const renderCommentItem = (comment: Comment, level: number = 0) => {
    return (
      <View key={comment.id} style={{ marginLeft: level * 16 }} className="mt-1">
          ...
          <View>
            {user && comment.user_id === user.id ? (
              <TouchableOpacity onPress={() => { setEditingCommentId(comment.id); setEditingContent(comment.content); }}>
                <Text className="text-base text-[#3b302a] dark:text-[#d4bfa3]">{comment.content}</Text>
              </TouchableOpacity>
            ) : (
              <Text className="text-base text-[#3b302a] dark:text-[#d4bfa3]">{comment.content}</Text>
            )}
        ...
        {comments.filter((c) => c.parent_id === comment.id).map((reply) => renderCommentItem(reply, level + 1))}
      </View>
    );
  };
...
export function LikeButton({ entityId, entityType = "post" }: LikeButtonProps) {
  ...
  return (
    <TouchableOpacity onPress={toggleLike} className="p-1">
      <Text className={`text-xs ${liked ? "text-blue-500" : "text-[#3b302a] dark:text-[#d4bfa3]"}`}>
        👍 {likeCount}
      </Text>
    </TouchableOpacity>
  );
...
# Updated: Removed separate reply input so that replies are handled in CommentsSection.
cat > "app/(tabs)/home.tsx" << 'EOF'
import React, { useEffect, useState } from 'react';
import { View, Text, FlatList, ActivityIndicator, RefreshControl, Alert, Platform, TouchableOpacity } from 'react-native';
import { useRouter } from 'expo-router';
import { supabase } from '../../src/lib/supabase';
import { useAuthStore } from '../../src/store/useAuthStore';
    ...
    try {
      ...
      if (error) {
        ...
        const enriched = await Promise.all(
          data.map((post: Post) => fetchProfileForPost(post))
        );
        setPosts(enriched);
      }
    } catch (err: any) {
  ...
  const renderPost = ({ item }: { item: Post }) => {
    ...
    return (
      <View className="p-4 border-b border-gray-300">
        ...
        {/* Render CommentsSection which handles nested replies */}
        <CommentsSection postId={item.id} hidePostReply={false} />
        {postErrors[item.id] && (
...
echo "7. Run and test the auth, messenger, and post flow."
...
1. **Script 1 (SQL):**  
   Run this script in your Supabase SQL editor (or as a migration) to initialize the backend schema—including RLS policies and realtime publications.

2. **Script 2 (Bash):**  
   - Save this script (e.g., as `setup.sh`), make it executable with `chmod +x setup.sh`, and run it from your project root.
   - Ensure your `.env` file contains the correct values for `EXPO_PUBLIC_SUPABASE_URL`, `EXPO_PUBLIC_SUPABASE_ANON_KEY`, and `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY`.
   - When the script finishes, it will launch Expo with:
     ```bash
     npx expo start --clear
     ```

3. **Script 3 (SQL):**  
   Run this script in your Supabase SQL editor to seed sample data (admin, testers, conversations, messages, and a multi‑line post).

After running these scripts, sign in as one of your testers (for example, `testuser@example.com`). The Home screen will display only top‑level posts (rows in “contents” where `parent_id` is null) with each post rendering its own CommentsSection. In CommentsSection, tapping the **Reply** button will open a reply input and when you submit your reply, it will be inserted with its `parent_id` set to the parent post’s id (or the parent comment’s id if replying to a comment) and displayed nested under the appropriate post.
