Below are the three complete, updated scripts with the nested‐reply issue fixed. In the CommentsSection component, we now cast the IDs to strings when filtering so that replies are properly recognized as children of their parent comment rather than appearing as top‑level comments.

Please copy these scripts into your respective files and run them as described.
...
Save this as (for example) `setup.sh`, make it executable with `chmod +x setup.sh`, and run it from your project root. (This script ends by launching Expo with `npx expo start --clear`.)
...
# Updated: When filtering, we now compare IDs as strings to avoid type mismatches.
...
export function CommentsSection({ postId, hidePostReply = false }: CommentsSectionProps) {
  ...
  // Fetch all comments (all rows in "contents" where parent_id is not null)
  ...
  // Submit a new comment. If replying to a comment, set parent_id to that comment's id; otherwise, to the post's id.
  ...
  // Render a comment and recursively render its replies (using parent_id).
  // We now compare IDs as strings to ensure type consistency.
  const renderCommentItem = (comment: Comment, level: number = 0) => {
    const replies = comments.filter((c) => String(c.parent_id) === String(comment.id));
    return (
  ...
  // Top-level comments are those whose parent_id equals the postId.
  const topLevelComments = comments.filter((c) => String(c.parent_id) === String(postId));
...
echo "2. Run Script 3 in your Supabase SQL editor to seed sample data."
echo "3. Then, the setup script will automatically launch Expo using:"
...
Run this script in your Supabase SQL editor after running Script 1.
...
2. **Script 2 (Bash):**  
   ...
   - Ensure your `.env` file has the correct values for `EXPO_PUBLIC_SUPABASE_URL`, `EXPO_PUBLIC_SUPABASE_ANON_KEY`, and `EXPO_PUBLIC_GOOGLE_MAPS_API_KEY`.  
   - When the script finishes, it will automatically launch Expo with:
     ```bash
     npx expo start --clear
     ```
...
After running these scripts, sign in as one of your testers (for example, `testuser@example.com`) and verify that:  
– Top‑level posts (rows in “contents” where `parent_id` is null) appear on the Home screen.  
– Replies you submit via the CommentsSection component are nested under the post or parent comment as intended.

If you still encounter issues, please verify that the inserted reply’s `parent_id` matches exactly (as strings) the parent comment’s `id`. This updated code should resolve the nesting problem.
